...
import { encodeMapPropertyColumn } from "./mapPropertyEncoder";
import { encodeBooleanColumn, encodeBooleanNullableColumn, encodeDoubleColumn, encodeDoubleNullableColumn, encodeFloatColumn, encodeFloatNullableColumn, encodeInt32NoneColumn, encodeInt32NullableColumn, encodeInt64NoneColumn, encodeInt64NullableColumn, encodeUint32Column, encodeUint64Column, encodeUint64NullableColumn, } from "./propertyEncoder";
 ...
 */
export function encodeTile(layers, options = {}) {
...
function encodeLayer(layer, options) {
    ...
    const body = concatenateBuffers(encodeVarintValue(TAG), encodeFieldName(layer.name), encodeVarintValue(extent), encodeVarintValue(columns.length), ...metadata, ...columns);
...
function collectPropertyNames(features) {
    const names = [];
...
function inferPropertyType(name, values) {
    ...
    if (present.some((value) => typeof value === "object"))
        return "map";
    ...
    if (present.every((value) => typeof value === "number")) {
        if (!present.every((value) => Number.isSafeInteger(value)))
            return "double";
 ...
 */
function encodeIdColumn(features) {
...
function encodePropertyColumn(type, values, nullable) {
    if (type === "string") {
    ...
    if (type === "int64" || type === "uint64") {
        const bigints = values.map((value) => (value === null ? null : BigInt(value)));
        if (type === "uint64") {
 ...
 */
function encodeGeometryColumn(features) {
    ...
    if (hasMulti)
        streams.push(lengthStream(LengthType.GEOMETRIES, geometryLengths));
    if (hasMulti || hasPolygon || hasLine)
        streams.push(lengthStream(LengthType.PARTS, partLengths));
    if (hasPolygon)
        streams.push(lengthStream(LengthType.RINGS, ringLengths));
    streams.push(createStream(PhysicalStreamType.DATA, encodeVarints(zigZag(vertices)), {
...
function lengthStream(lengthType, values) {
...
function geometryTypeOf(geometry) {
...
function encodeVarintValue(value) {
    const buffer = new Uint8Array(5);
    const offset = new IntWrapper(0);
    encodeVarintInt32Value(value, buffer, offset);
    return buffer.slice(0, offset.get());
}
//# sourceMappingURL=mltEncoder.js.map