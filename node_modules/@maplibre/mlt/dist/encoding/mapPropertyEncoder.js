...
import { encodeDoubleColumn, encodeFloatColumn, encodeInt32NoneColumn, encodeInt64NoneColumn, encodeUint32Column, encodeUint64Column, } from "./propertyEncoder";
 ...
 */
export function encodeMapPropertyColumn(childColumns, options = {}) {
 ...
 */
function collectDictionaries(childColumns, options) {
    const strings = [];
    ...
    const collectInteger = (value) => {
        if (options.unsignedIntegers || value > INT64_MAX)
            unsignedIntegers.push(value);
        else
            signedIntegers.push(value);
    };
    const collect = (value) => {
        ...
        if (typeof value === "string")
            ...
            collectInteger(value);
        else if (isIntegral(value))
            collectInteger(BigInt(value));
        else
            decimals.push(value);
    };
    for (const column of childColumns) {
    ...
    for (const value of strings)
        indexByKey.set(dictionaryKey(value), index++);
    for (const value of signedIntegers)
        indexByKey.set(dictionaryKey(value), index++);
    for (const value of unsignedIntegers)
        indexByKey.set(dictionaryKey(value), index++);
 ...
 */
function encodeChildColumn(column, streams) {
    for (const value of column) {
 ...
 */
function encodeFeatureValue(value, flattenedValues, indexByKey) {
...
function encodeMapEntries(value, flattenedValues, indexByKey) {
    for (const [key, entry] of Object.entries(value)) {
        flattenedValues.push(encodeScalarByIndex(key, indexByKey));
        encodeValue(entry, flattenedValues, indexByKey);
    }
}
function encodeValue(value, flattenedValues, indexByKey) {
    rejectNestedNull(value);
    if (typeof value === "boolean") {
 ...
 */
function encodeNestedPayloadLength(flattenedValues, startIndex) {
...
function encodeScalarByIndex(value, indexByKey) {
...
function encodeMapStreams(streams, options) {
    const parts = [];
    let mask = 0;
    let numStreams = 0;
    const write = (written) => {
...
function encodeStringDictionary(strings, parts) {
    if (strings.length === 0) {
 ...
 */
function encodeIntegerDictionaries(dictionaries, parts) {
...
function encodeSignedIntegerDictionary(integers, parts) {
...
function encodeUnsignedIntegerDictionary(integers, parts) {
    if (integers.length === 0) {
        return { mask: 0, count: 0 };
    }
...
function encodeFloatingPointDictionaries(decimals, parts, options) {
 ...
 */
function encodePresenceStream(presenceBits, parts) {
    if (!presenceBits.includes(false)) {
        return { mask: 0, count: 0 };
    }
    parts.push(createStream(PhysicalStreamType.PRESENT, encodeBooleanRle(presenceBits), { count: presenceBits.length }));
 ...
 */
function rejectNestedNull(value) {
...
//# sourceMappingURL=mapPropertyEncoder.js.map