import { refProperties } from "./util/ref_properties.mjs";
...
function stringify(obj) {
	...
	if (Array.isArray(obj)) {
		let str = "[";
		for (const val of obj) str += `${stringify(val)},`;
		return `${str}]`;
	}
	const keys = Object.keys(obj).sort();
	let str = "{";
	for (let i = 0; i < keys.length; i++) str += `${JSON.stringify(keys[i])}:${stringify(obj[keys[i]])},`;
	return `${str}}`;
}
function getKey(layer) {
	let key = "";
	for (const k of refProperties) key += `/${stringify(layer[k])}`;
	return key;
}
/**
...
//#endregion
export { groupByLayout };

//# sourceMappingURL=group_by_layout.mjs.map