...
const darkTiles = L.tileLayer(
  ...
  initialLayer = isDark ? darkTiles : lightTiles,
  map = L.map("map", { zoomControl: !1, layers: [initialLayer] }).setView(
    [51.505, -0.09],
    13,
  ),
  baseMaps = {
...
function copyWithFeedback(e, t, a) {
...
async function fetchSmartIPs() {
  const e = document.getElementById("ip-display-area");
  try {
    const t = await fetch("/api/config"),
      a = await t.json();
    let s = "/api/info";
    const n = window.location.search.substring(1).trim();
    let r = null;
    if (n) {
      const e = new URLSearchParams(window.location.search);
      e.has("ip") ? (r = e.get("ip")) : n.includes("=") || (r = n);
    }
    if (r) {
      s = `/api/info?ip=${r}`;
    ...
    const l = d.ip.includes(":"),
      i = l ? "IPv6" : "IPv4";
    if (((e.innerHTML = createIpRow(d.ip, i, !0)), populateDetails(d), !r)) {
      const t = l ? a.v4_url : a.v6_url,
        s = l ? "IPv4" : "IPv6";
      try {
        const a = await fetch(t);
        if (a.ok) {
          const t = await a.json();
          e.innerHTML += createIpRow(t.ip, s, !1);
        }
      } catch (e) {
    ...
    ((e.innerHTML = '<span class="text-red-400">Error loading IP</span>'),
      console.error(t));
  }
}
function populateDetails(e) {
  ((currentScanIp = e.ip),
    resetReputationUI(),
    (document.getElementById("dataOrg").innerText = e.org || "N/A"),
    (document.getElementById("dataAsn").innerText = e.asn || "N/A"),
    (document.getElementById("dataCity").innerText = e.city),
    (document.getElementById("mainLocation").innerText =
      `${e.city}, ${e.country}`),
    (document.getElementById("dataRegion").innerText =
      `${e.region}, ${e.country}`),
    e.zip && "N/A" !== e.zip && "-" !== e.zip
      ? ((document.getElementById("dataZip").innerText = e.zip),
        (document.getElementById("zipWrapper").style.display = "inline"))
      : (document.getElementById("zipWrapper").style.display = "none"),
    (document.getElementById("dataTimezone").innerText = e.timezone),
    (document.getElementById("dataCoords").innerText = e.coordinates),
    (document.getElementById("jsonPreview").innerHTML = syntaxHighlight(e)));
  const t = document.getElementById("dataProxyBadge");
  if (
    (e.is_proxy
      ? ((t.innerText = e.proxy_type),
        (t.className =
      ...
      : "Residential" === e.usage_type || "Mobile Data" === e.usage_type
        ? ((t.innerText = e.usage_type),
          (t.className =
...
async function searchIp() {
  ...
  if (e)
    try {
      const t = await fetch(`/api/info?ip=${e}`),
        a = await t.json();
      populateDetails(a);
      const s = e.includes(":") ? "IPv6" : "IPv4";
...
function copyText(e) {
  navigator.clipboard
    .writeText(document.getElementById(e).innerText.replace("$ ", ""))
    .then(() => showToast("Command Copied!"));
}
function resetReputationUI() {
  const e = document.getElementById("btn-scan");
  (e &&
    (e.classList.remove("hidden"),
    (e.disabled = !1),
    (e.innerHTML =
      '<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> SCAN THREATS')),
    document.getElementById("rep-badge").classList.add("hidden"),
    (lastReputationResult = null));
}
async function checkReputation() {
  const e = document.getElementById("btn-scan"),
    t = document.getElementById("rep-badge");
  ((e.disabled = !0),
    (e.innerHTML =
      '<svg class="animate-spin h-3 w-3 mr-2" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg> SCANNING...'));
  try {
    const a = await fetch(`/api/reputation?ip=${currentScanIp}`),
      s = await a.json();
    if (
      ((lastReputationResult = s),
      e.classList.add("hidden"),
      (e.disabled = !1),
      t.classList.remove("hidden"),
      (t.onclick = openReputationModal),
      (t.style.cursor = "pointer"),
      s.is_clean)
    )
      ((t.innerHTML =
        'NO THREATS DETECTED <span class="text-lg leading-none">ⓘ</span>'),
        (t.className =
      ...
      if (!s.detections) throw new Error("Invalid API response");
      ((t.innerText = `${s.detections.length} THREATS FOUND ⚠️`),
        (t.className =
    ...
    (console.error(t), (e.innerHTML = "ERROR - TRY AGAIN"), (e.disabled = !1));
  }
}
function openReputationModal() {
  if (!lastReputationResult) return;
  const e = document.getElementById("rep-modal"),
    t = document.getElementById("modal-content"),
    a = document.getElementById("rep-modal-backdrop"),
    s = document.getElementById("rep-modal-panel");
  if ((e.classList.remove("hidden"), lastReputationResult.is_clean)) {
    const e = `IP: ${lastReputationResult.ip} - Status: CLEAN (No threats detected in active feeds)`;
    ...
    let e = "",
      a = [];
    lastReputationResult.detections.forEach((t) => {
      (a.push(t.source),
...
function closeReputationModal() {
  const e = document.getElementById("rep-modal"),
    t = document.getElementById("rep-modal-backdrop"),
    a = document.getElementById("rep-modal-panel");
...
async function checkWhois() {
  const e = document.getElementById("btn-whois"),
    t = document.getElementById("whois-content");
  ((e.disabled = !0), (e.innerText = "LOADING..."));
  try {
    const e = await fetch(`/api/whois?ip=${currentScanIp}`),
      a = await e.json();
    if (a.error) throw new Error(a.error);
    if (a.is_raw) {
      const e = /([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)/gi;
      let s = a.raw_whois
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
        .replace(
    ...
    const s = document.getElementById("whois-modal"),
      n = document.getElementById("whois-modal-backdrop"),
      r = document.getElementById("whois-modal-panel");
    (s.classList.remove("hidden"),
      setTimeout(() => {
        (n.classList.remove("opacity-0"),
          r.classList.remove("opacity-0", "scale-95"),
          r.classList.add("opacity-100", "scale-100"));
      }, 10));
  } catch (e) {
    showToast("WHOIS lookup failed. Registry might be rate limiting.");
  } finally {
    ((e.innerText = "WHOIS"), (e.disabled = !1));
  }
}
function closeWhoisModal() {
  const e = document.getElementById("whois-modal"),
    t = document.getElementById("whois-modal-backdrop"),
    a = document.getElementById("whois-modal-panel");
  (t.classList.add("opacity-0"),
    a.classList.add("opacity-0", "scale-95"),
    a.classList.remove("opacity-100", "scale-100"),
    setTimeout(() => {
...
(fetchSmartIPs(),
  document.addEventListener("keydown", (e) => {
    if ("Escape" === e.key) {
      const e = document.getElementById("rep-modal"),
        t = document.getElementById("whois-modal");
      (e && !e.classList.contains("hidden") && closeReputationModal(),
        t && !t.classList.contains("hidden") && closeWhoisModal());
    }
  }));
const repModal = document.getElementById("rep-modal");
repModal &&
  repModal.addEventListener("click", (e) => {
    ("rep-modal" !== e.target.id && "rep-modal-backdrop" !== e.target.id) ||
      closeReputationModal();
  });
const whoisModal = document.getElementById("whois-modal");
function syntaxHighlight(e) {
  return (
    "string" != typeof e && (e = JSON.stringify(e, void 0, 2)),
    (e = e
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;")).replace(
      ...
      function (e) {
        let t = "text-blue-400 dark:text-blue-300";
        return (
          /^"/.test(e)
            ? (t = /:$/.test(e)
...
whoisModal &&
  whoisModal.addEventListener("click", (e) => {
    ("whois-modal" !== e.target.id && "whois-modal-backdrop" !== e.target.id) ||
      closeWhoisModal();
  });
